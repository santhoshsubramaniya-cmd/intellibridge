            ...
            Row(children: [
              Expanded(
                child: Column(
                    ...
                    children: [
                      _buildLabel(
                          'Min CGPA: ${_minCgpa.toStringAsFixed(1)}'),
                      Slider(
                          value: _minCgpa,
                          min: 5,
                          max: 10,
              ...
              Expanded(
                child: Column(
                    ...
                    children: [
                      _buildLabel(
                          'Percentile Cutoff: ${_percentileCutoff.toInt()}%'),
                      Slider(
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(_isPosting
                    ? 'Posting & Alerting Students...'
                    : 'Post Drive & Alert Students'),
              ),
            ),

            const SizedBox(height: 32),
            const Text('Posted Drives',
                    style: TextStyle(
                        fontFamily: 'Syne',
                        fontSize: 18,
                        fontWeight: FontWeight.w700))
            ...
            StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                final drives = snapshot.data!.docs
                    .map((d) => PlacementDrive.fromMap(
                        d.data() as Map<String, dynamic>, d.id))
                    .toList();
                if (drives.isEmpty) {
                  return const Text('No drives posted yet',
                      style: TextStyle(color: AppColors.lightMuted));
                }
                return Column(
                    children: drives
                        .map((drive) => _DriveListCard(drive: drive))
                        .toList());
              },
            ),
  ...
  Future<void> _postDrive() async {
    if (_companyCtrl.text.isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Please fill company, role, and visit date')));
      return;
    }
    ...
    for (final student in students) {
      final alertMsg = await _gemini.generateSmartAlert(
        ...
        requiredPercentile: _percentileCutoff.toInt(),
        daysLeft: daysLeft,
        skillGaps: _selectedSkills.take(2).toList(),
      );
      ...
      await _fs.createAlert(
        ...
        title:
            '🏢 ${_companyCtrl.text.trim()} Drive — $daysLeft days away!',
        message: alertMsg,
      );
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            '✅ Drive posted! AI alerts sent to ${students.length} students.'),
    ...
    return Container(
      ...
      child: Row(children: [
        Expanded(
          child: Column(
              ...
              children: [
                ...
                Text(
                    '${drive.visitDate.day}/${drive.visitDate.month}/${drive.visitDate.year} · CGPA ≥${drive.minCgpa}',
                    style: const TextStyle(
        ...
        Container(
          ...
          decoration: BoxDecoration(
              color: drive.isUpcoming
                  ? AppColors.secondary.withOpacity(0.1)
                  : AppColors.lightMuted.withOpacity(0.1),
              borderRadius: BorderRadius.circular(8)),
          child: Text(
              drive.isUpcoming
